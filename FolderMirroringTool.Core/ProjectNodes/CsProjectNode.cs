...
namespace FolderMirroringTool.Core.ProjectNodes
{
    ...
    public abstract class CsProjectNode
    {
        ...
        {
            Node = new XElement(XName.Get(ElementName));
        }
        ...
        public void AddAfterSelf(CsProjectNode node)
        {
            ...
            if (node.Depth == 1)
                node.Node.AddBeforeSelf(Environment.NewLine);

            node.Node.AddBeforeSelf(new XText(Environment.NewLine + new string(' ', node.Depth * 2)));
        }
        ...
        {
            return new Element(Element.Element(XName.Get(name)), Depth + 1);
        }
